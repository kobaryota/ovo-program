...
open(IN0,"@ARGV[0]");
$file = @ARGV[0];
($mae,$file2) = split (/SiO\/v2\//, $file);
($name,$ushiro) = split (/\_Qv2\_/, $file2);
while ($line = <IN0>) {
	chomp $line;
	if($line =~ /\#/){
		$date = $line;
...
#	$plot[$k++] = sprintf("plot \"$file\" ind 0 u 1:2 ti \"\" w l\n");
	...
	$plot[$k++] = sprintf("set output '/web/OVO/data/$name/SiO/v2/$name\_$date2.png'\n");
	$plot[$k++] = sprintf("plot \"/web/OVO/data/$name/SiO/v2/$file2\" u 1:2 ti \"\" w l\n");
