...
$filesp = @ARGV[0];
$file = $filesp;
($mae,$ushiro) = split (/\_Qv1\_/, $file);
($mae2,$name) = split (/v1\//, $mae);
$wc = `wc -l $filesp`;
($s,$gyosu) = split(/\s+/, $wc);
while ($line = <IN0>) {
	chomp $line;
...
sub make_gnuplot{
	...
	$plot[$k++] = sprintf("set title \"SiO Maser's v=1 Spectrum of $date\"\n");
	$plot[$k++] = sprintf("set key outside\n");
        $plot[$k++] = sprintf("set terminal png\n");
	$plot[$k++] = sprintf("set output '$Ddat/$name/SiO/v1/$name\_$date2.png'\n");
